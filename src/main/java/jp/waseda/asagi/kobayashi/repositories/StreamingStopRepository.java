...
public class StreamingStopRepository extends Thread {
  private final String roomID;
  private final Consumer<OriginalResult<Boolean>> callback;

  public StreamingStopRepository(String roomID, Consumer<OriginalResult<Boolean>> callback) {
    this.roomID = roomID;
  ...
  private Boolean stop(String roomID) throws IOException, UnknownException {
    final String request = RequestParser.stop(roomID);
    final String responce = ServerClient.getInstance().send(request);
    final boolean success = ResponceParser.stop(responce);
    return success;
  }
}
